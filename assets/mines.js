...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-ab4cd11d";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-ab4cd11d">'+'<span class="tile-face-ab4cd11d tile-front-ab4cd11d"></span>'+'<span class="tile-face-ab4cd11d tile-back-ab4cd11d"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ab4cd11d")){return;}
var back=tile.querySelector(".tile-back-ab4cd11d");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-ab4cd11d");if(isMine){tile.classList.add("boom-ab4cd11d");}
if(dim){tile.classList.add("dimmed-ab4cd11d");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-ab4cd11d")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-ab4cd11d");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ab4cd11d")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-ab4cd11d");void panelEl.offsetWidth;panelEl.classList.add("jolt-ab4cd11d");}
endRound();}
